...
function SH({ icon: Icon, text }) {
    return (
        <div className="mt-8 sm:mt-10 md:mt-12 mb-3 sm:mb-4">
            <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-black text-white font-mono font-black uppercase text-xs sm:text-sm border-2 border-black shadow-[3px_3px_0_rgba(0,0,0,0.2)] sm:shadow-[4px_4px_0_rgba(0,0,0,0.2)]">
                <Icon size={14} className="sm:w-4 sm:h-4" /> {text}
            </div>
...
export default function Analytics() {
    ...
    return (
        <div className="animate-fade-in w-full max-w-7xl xl:max-w-[1500px] mx-auto px-3 sm:px-4 md:px-6 pb-12 sm:pb-16">
            <div className="my-6 sm:my-8 md:my-10 text-center">
                <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black uppercase tracking-tight">
                    DEEP <span className="bg-[#CEFFBC] border-[3px] sm:border-4 border-black px-2 sm:px-3 py-1 shadow-[4px_4px_0_#000] sm:shadow-[6px_6px_0_#000] inline-block -rotate-1">ANALYSIS</span>
                </h1>
                <p className="mt-4 sm:mt-6 font-mono font-bold text-gray-500 uppercase flex items-center justify-center gap-2 text-xs sm:text-sm flex-wrap">
                    <Server size={14} /> Agent: <span className="text-black bg-yellow-100 px-2 border border-black break-all">{deviceName}</span>
                </p>
            </div>

            {loading ? (
                <div className="flex items-center justify-center gap-4 p-8 sm:p-12 border-[3px] sm:border-4 border-black bg-white shadow-[5px_5px_0_#000] sm:shadow-[8px_8px_0_#000]">
                    <Loader2 className="animate-spin" size={28} />
                    <p className="font-mono font-black uppercase text-sm sm:text-base">Scanning Deep Metrics…</p>
                </div>
            ) : (
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 mt-4 font-mono text-[10px] uppercase font-bold">
                    ...
                    {(m.c2_communication_score > 0.5 || m.data_exfiltration_score > 0.5 || m.port_scanning_score > 0.5) && (
                        <div className="mt-6 border-[3px] sm:border-4 border-red-600 bg-red-100 p-3 sm:p-4 shadow-[5px_5px_0_#dc2626] sm:shadow-[8px_8px_0_#dc2626] flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 animate-pulse text-center sm:text-left">
                            <AlertCircle size={28} className="text-red-700 sm:w-8 sm:h-8 flex-shrink-0" />
                            <p className="font-mono text-base sm:text-lg md:text-xl font-black text-red-700 uppercase leading-tight sm:leading-none">High Threat Score Detected — Deep investigation recommended</p>
                        </div>
                    )}
                </div>
            )}
