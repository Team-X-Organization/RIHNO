...
function Notification() {
    ...
    return (
        <div className="flex flex-col items-center animate-fade-in w-full p-6 min-h-screen bg-white font-sans text-black">

            {/* Header */}
            <div className="mb-12 w-full max-w-6xl flex flex-col items-center">
                <h1 className="text-5xl md:text-7xl font-black uppercase leading-none text-center mb-6">
                    OVERVIEW
                    <span className="block md:inline-block bg-[#CEFFBC] border-[4px] border-black px-4 ml-0 md:ml-4 shadow-[8px_8px_0_#000]">
                        CENTER
                    </span>
                </h1>
                ...
                <div className="flex flex-wrap justify-center gap-4 mt-4">
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-3 border-4 border-black font-mono font-black uppercase text-lg sm:text-xl transition-all shadow-[6px_6px_0_#000] hover:-translate-y-1 hover:shadow-[8px_8px_0_#000] ${activeTab === 'agents' ? 'bg-black text-white' : 'bg-white text-black'}`}>
                        <Server size={24} />
                        Agent Status
                    </button>
                    <button
                        onClick={() => setActiveTab('threats')}
                        className={`flex items-center gap-2 px-6 py-3 border-4 border-black font-mono font-black uppercase text-lg sm:text-xl transition-all shadow-[6px_6px_0_#000] hover:-translate-y-1 hover:shadow-[8px_8px_0_#000] ${activeTab === 'threats' ? 'bg-[#FF6B6B] text-white' : 'bg-white text-black'}`}>
                        <ShieldAlert size={24} />
                        IP Threats
                    </button>
                </div>
            ...
            {loading ? (
                <div className="flex items-center justify-center gap-4 p-12 border-4 border-black bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                    ...
                    <p className="font-mono text-2xl font-black uppercase tracking-widest">Gathering Telemetry…</p>
                </div>
            ) : (
                <div className="w-full max-w-6xl">
                    ...
                    {activeTab === 'agents' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {enrichedAgents.length === 0 ? (
                                <div className="col-span-full p-12 border-4 border-black bg-white shadow-[8px_8px_0_#000] text-center">
                                    <p className="font-mono text-xl font-black uppercase">No Agents Available.</p>
                                </div>
                            ) : (
                                enrichedAgents.map((agent, idx) => (
                                    <div key={idx} className="border-4 border-black bg-white shadow-[8px_8px_0_#000] hover:-translate-y-1 hover:shadow-[10px_10px_0_#000] transition-all flex flex-col h-full">
                    ...
                    {activeTab === 'threats' && (
                        <div className="space-y-6">
                            ...
                            <div className="p-8 border-4 border-black bg-[#FF6B6B] text-white shadow-[8px_8px_0_#000] flex flex-col md:flex-row items-center justify-between gap-6">
                                <div className="flex items-center gap-4">
                                    <ShieldAlert size={64} className="animate-pulse" />
                                    <div>
                                        <h2 className="text-3xl sm:text-5xl font-black uppercase">Threat Radar</h2>
                                        <p className="font-mono text-lg sm:text-xl font-bold uppercase mt-1">Cross-Agent IP Analysis</p>
                                    </div>
                                </div>
                                <div className="bg-black text-white px-8 py-4 border-4 border-white shadow-[6px_6px_0_#fff] text-center">
                                    <p className="font-mono text-sm font-bold uppercase">Total Threats</p>
                            ...
                            {/* Threat Detail List */}
                            {threatData.length === 0 ? (
                                <div className="p-12 border-4 border-black bg-[#CEFFBC] shadow-[8px_8px_0_#000] text-center">
                                    <Shield size={48} className="mx-auto mb-4 text-black" />
                                    <p className="font-mono text-2xl font-black uppercase text-black">Zero IP Threats Detected.</p>
                                </div>
                            ) : (
                                threatData.map((threat, idx) => (
                                    <div key={idx} className="border-4 border-black bg-white shadow-[8px_8px_0_#000] hover:-translate-y-1 hover:shadow-[10px_10px_0_#000] transition-all overflow-hidden flex flex-col md:flex-row">
                                        {/* Threat Indicator */}
                                        <div className="w-full md:w-48 bg-[#FF6B6B] border-b-4 md:border-b-0 md:border-r-4 border-black p-6 flex flex-col items-center justify-center text-white text-center">
